...
Estudiante::Estudiante(string nombre, int legajo)
    ...
    /*
    Voy a utilizar una shallow copy ya que esta crea una copia del vector estudiantes
    pero que sigue apuntando a los mismos alumnos en vez de tambien copiar a los alumnos.
    Eso no es necesario ya que podrian quedar alumnos iguales pero con distintas
    notas ya que serian distintos objetos que deberian ser el mismo entonces esta de mas copiarlos a ellos tambien.
    */
    //Constructor copia
    Curso::Curso(string name, const Curso& otro)
        : estudiantes(otro.estudiantes), nombre(name) {}
